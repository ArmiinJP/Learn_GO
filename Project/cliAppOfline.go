...
func textFileToMap() error {
	...
	for fileScanner.Scan() {
		...
		tmpAgency.Id, err = strconv.Atoi(tmpSlice[1]);if err != nil {return err}
		tmpAgency.Name = tmpSlice[2]
		tmpAgency.Address = tmpSlice[3]
		tmpAgency.phoneNumber = tmpSlice[4]
		tmpAgency.registeryDate = tmpSlice[5]
		tmpAgency.numberEmployee, err = strconv.Atoi(tmpSlice[6]);if err != nil {return err}
	...
	switch *command{
		...
		case "edit":{
			...
			checkValue := func(new string, old string) string {
				if new == "N" || new == "No" || new == "NO" || new == "no" || new == "n" {return old
				} else {return new}
			}
		...
		default: log.Fatalf("Error occurd!! Command %s not Found", *command)
	}
}